...
@patch('tasks._git.check_output', autospec=True)
def test_repository_add(check_output_mock, repository):
    repository.add(['src'])
    check_output_mock.assert_called_once_with(
        ['git', 'add', 'src'], cwd='/tmp')
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_add_multiple_filepaths(check_output_mock, repository):
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_authors_since_one(check_output_mock, repository):
    check_output_mock.return_value = 'example@example.com'
    authors = repository.authors_since('HEAD~1')
    check_output_mock.assert_called_once_with(
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_authors_since_multiple(check_output_mock, repository):
    check_output_mock.return_value = 'example@example.com\ntest@test.com'
    authors = repository.authors_since('HEAD~2')
    check_output_mock.assert_called_once_with(
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_commit(check_output_mock, repository):
    repository.commit('hello world', 'example name', 'example@example.com')
    check_output_mock.assert_called_once_with(
        ['git',
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_diff_name_status_none(check_output_mock, repository):
    check_output_mock.return_value = ''
    pairs = repository.diff_name_status()
    check_output_mock.assert_called_once_with(
...
def test_repository_diff_name_status_none_staged(check_output_mock,
                                                 repository):
    check_output_mock.return_value = ''
    pairs = repository.diff_name_status(staged=False)
    check_output_mock.assert_called_once_with(
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_diff_name_status_none_rev(check_output_mock, repository):
    check_output_mock.return_value = ''
    pairs = repository.diff_name_status(rev='HEAD~1')
    check_output_mock.assert_called_once_with(
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_diff_name_status_one(check_output_mock, repository):
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_diff_name_status_multiple(check_output_mock, repository):
    check_output_mock.return_value = ('A\t/tmp/hello.txt\n'
                                      'D\t/tmp/world.txt\n'
                                      'M\t/tmp/foo.txt\n'
                                      'X\t/tmp/bar.txt')
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_latest_tag(check_output_mock, repository):
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_push(check_output_mock, repository):
    repository.push()
    check_output_mock.assert_called_once_with(
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_push_remote_branch(check_output_mock, repository):
    repository.push(remote='github', branch='dev')
    check_output_mock.assert_called_once_with(
        ['git', 'push', 'github', 'dev'], cwd='/tmp')


@patch('tasks._git.check_output', autospec=True)
def test_repository_push_tags(check_output_mock, repository):
...
@patch('tasks._git.check_output', autospec=True)
def test_repository_soft_reset(check_output_mock, repository):
    repository.soft_reset('HEAD~1')
    check_output_mock.assert_called_once_with(
        ['git', 'reset', '--soft', 'HEAD~1'], cwd='/tmp')


@patch('tasks._git.check_output', autospec=True)
def test_repository_tag(check_output_mock, repository):
    repository.tag('v0.1')
    check_output_mock.assert_called_once_with(
