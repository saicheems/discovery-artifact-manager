...
from tasks import _accounts, _git, google_api_ruby_client
from tests import helpers

_GITHUB_ACCOUNT = _accounts.GitHubAccount('Test', 'test@test.com', '_', '_')
_RUBYGEMS_ACCOUNT = _accounts.RubyGemsAccount('api_key')
...
def test_update(clone_from_github_mock, check_output_mock, date_mock):
    ...
    google_api_ruby_client.update('/tmp', _GITHUB_ACCOUNT)
...
def test_release_minor(clone_from_github_mock,
    ...
    open_version_rb_mock = mock_open(
        read_data=('...\n'
                   ...
                   '        VERSION = \'0.13.6\'\n'
                   '    ...\n'))
    open_changelog_md_mock = mock_open(read_data='# 0.13.6\n...\n')
    ...
    google_api_ruby_client.release('/tmp', _GITHUB_ACCOUNT, _RUBYGEMS_ACCOUNT)
    assert manager.mock_calls == [
        call.clone_from_github('google/google-api-ruby-client',
                               '/tmp/google-api-ruby-client',
                               github_account=_GITHUB_ACCOUNT),
...
def test_release_no_commits_since_latest_tag(clone_from_github_mock):
    ...
    with pytest.raises(Exception) as excinfo:
        google_api_ruby_client.release(
            '/tmp', _GITHUB_ACCOUNT, _RUBYGEMS_ACCOUNT)
