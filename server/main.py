...
def _git_commit(cwd, message, github_account, check=False):
...
def _git_push(cwd, remote='origin', branch='master', tags=False):
    """Pushes all commits to the repo at cwd.
...
def _verify_git_log(cwd, latest_tag, github_account):
    ...
    Returns true if there is at least one commit since the `latest_tag` and all
...
def cron_discoveries():
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        returncode = _git_commit(dartman_dir, commitmsg, account)
...
def cron_clients_go_update():
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        _call('make all', check=True, cwd=generator_dir, env=env)
        ...
        commitmsg = _build_commitmsg(added, deleted, updated, subject=subject)
...
def cron_clients_nodejs_update():
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        _git_clone(Repo.NODEJS, account, client_lib_dir)
        ...
        # Stage all changes.
        _call('git add apis', check=True, cwd=client_lib_dir)
...
def cron_clients_php_update():
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        client_lib_dir = os.path.join(
            tmp_dir, 'google-api-php-client-services')
        ...
        for filename in discovery_document_filenames:
            ...
            # A zero return code means there's something to push.
            if _git_commit(client_lib_dir, '', account) == 0:
                commit_count += 1
